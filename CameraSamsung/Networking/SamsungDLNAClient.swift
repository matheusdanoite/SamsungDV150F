...
final class SamsungDLNAClient {
    ...
    private var deviceID: String {
        // Many Samsung cameras expect a 12-char hex string (MAC-like).
        // Using the first 12 chars of the dash-less UUID.
        String(getLocalMAC().replacingOccurrences(of: "-", with: "").prefix(12))
    }
    ...
    func subscribeToEvents() async {
        // Busca o serviço de ContentDirectory
        guard let contentDirectoryService = cameraInfo?.services.first(where: { $0.serviceType.contains("ContentDirectory") }),
            !contentDirectoryService.eventSubURL.isEmpty else {
            return
        }
        
        let urlString = "\(baseURL)\(contentDirectoryService.eventSubURL)"
        guard let url = URL(string: urlString) else { return }
        
        var request = URLRequest(url: url)
        request.httpMethod = "SUBSCRIBE"
        
        // O iOS precisa informar um CALLBACK, mesmo que seja um endpoint local "dummy", 
        // pois a câmera valida a presença deste cabeçalho.
        let localIP = AutoShareClient.getLocalIP() ?? "192.168.101.2"
        request.setValue("<http://\(localIP):1801/events>", forHTTPHeaderField: "CALLBACK")
        ...
        do {
            ...
            let sid = httpResponse.value(forHTTPHeaderField: "SID") {
                ...
                print("Inscrição UPnP aceita. SID: \(sid)")
            }
        } catch {
            print("Falha na inscrição UPnP: \(error.localizedDescription)")
        }
    }
    ...
    func connect() async throws {
        ...
        // Immediate event subscription is critical for Samsung idle timer
    ...
    private func fetchFeatureList() async {
        let soapAction = "urn:schemas-upnp-org:service:ContentDirectory:1#X_GetFeatureList"
        let soapBody = """
        ...
        log(.sent, "SOAP X_GetFeatureList → \(url)")
        
        do {
            let (data, response) = try await httpPost(url: url, body: soapBody, soapAction: soapAction)
            if response.statusCode == 200 {
                let xml = decodeData(data)
                log(.received, "X_GetFeatureList response: \(xml)")
            } else {
                log(.info, "X_GetFeatureList failed: HTTP \(response.statusCode)")
            }
        } catch {
            log(.error, "X_GetFeatureList error: \(error.localizedDescription)")
        }
    }
    ...
    func registerClient() async {
        ...
        for soapAction in actions {
            ...
            let targetPaths = [controlURL, "/smp_11_", "/smp_4_", "/smp_8_"].filter { !$0.isEmpty }
            for targetPath in Set(targetPaths) {
                let url = "\(baseURL)\(targetPath)"
                let userAgent = "SEC_RVF_\(deviceID)"
                log(.sent, "Handshake Attempt: \(actionName) → \(url) (ID: \(deviceID))")
                ...
                do {
                    ...
                    if httpResponse.statusCode == 200 {
                        log(.info, "Handshake successful with \(actionName) at \(targetPath)!")
                        return
                    } else {
                        let errorBody = decodeData(data)
                        log(.info, "Handshake \(actionName) failed at \(targetPath): HTTP \(httpResponse.statusCode)")
                        if !errorBody.isEmpty && errorBody.count < 1000 {
                            log(.received, "Error body: \(errorBody)")
                        }
                    }
                } catch {
                    log(.error, "Handshake \(actionName) error at \(targetPath): \(error.localizedDescription)")
                }
            }
        }
    }
    ...
    heartbeatTimer?.invalidate()
        ...
        await MainActor.run {
            heartbeatTimer = Timer.scheduledTimer(withTimeInterval: 60, repeats: true) { [weak self] _ in
                Task {
                    guard let self = self, let sid = self.subscriptionID,
                        let service = self.cameraInfo?.services.first(where: { $0.serviceType.contains("ContentDirectory") }) else {
                        // Tenta se inscrever caso ainda não tenha SID
                        await self?.subscribeToEvents()
                        return
                    }
                
                    let urlString = "\(self.baseURL)\(service.eventSubURL)"
                    ...
                    if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 {
                        // Inscrição renovada com sucesso e timeout evitado
                    } else {
                        // O SID expirou ou a câmera rejeitou, força nova inscrição
    ...
    private func getLocalMAC() -> String {
        // iOS doesn't allow reading real MAC; use identifierForVendor as a stable UUID
        UIDevice.current.identifierForVendor?.uuidString ?? "00000000-0000-0000-0000-000000000000"
    }
}
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        ...
        if let current = currentItem {
            ...
            case "res":
                ...
                // Identificar se a tag atual é uma miniatura através do protocolInfo
                if tempMimeType.contains("JPEG_TN") || tempMimeType.contains("JPEG_SM") {
                    currentItem?.thumbnailURL = text
                } 
                // Heurística atual: Preferir o recurso com o maior tamanho para a imagem completa
                else if current.url.isEmpty || tempSize > current.size {
